...
const STORAGE_KEY = 'devotions_prayers'
...
export default function PrayersPage() {
  ...
  const loadPrayers = () => {
    try {
      ...
      if (saved) {
        const prayersObj = JSON.parse(saved) as Record<string, string>
        // Convert to array and sort by date (newest first)
        const entries: PrayerEntry[] = Object.entries(prayersObj)
          .map(([date, prayer]) => ({ date, prayer }))
  ...
  return (
    <div className="min-h-screen bg-navy-900 pb-20">
      <BackButton label="Saved Prayers" />
        ...
        {prayers.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16">
            <div className="w-16 h-16 bg-navy-800/40 border border-gold-500/20 rounded-full flex items-center justify-center mb-4">
              <Heart className="w-8 h-8 text-gold-500/50" />
            </div>
            <h3 className="text-lg font-semibold text-slate-50 mb-2">Nothing saved yet</h3>
            <p className="text-slate-400 text-sm text-center max-w-sm">
              Prayers you save from daily readings will appear here.
            </p>
            <button

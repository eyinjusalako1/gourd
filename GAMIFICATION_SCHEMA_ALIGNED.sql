...
-- ALIGNED WITH GATHERED SUPABASE SETUP
...
CREATE TABLE IF NOT EXISTS faith_flames (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  fellowship_id UUID REFERENCES fellowship_groups(id) ON DELETE CASCADE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_faith_streaks_active ON faith_streaks(fellowship_id, current_streak DESC) 
...
CREATE TABLE IF NOT EXISTS unity_points (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  fellowship_id UUID REFERENCES fellowship_groups(id) ON DELETE CASCADE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_unity_points_fellowship ON unity_points(fellowship_id, week_start DESC);
CREATE INDEX IF NOT EXISTS idx_unity_points_on_fire ON unity_points(fellowship_id) WHERE is_on_fire = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_unity_contributions_points ON unity_contributions(unity_points_id);
...
CREATE INDEX IF NOT EXISTS idx_weekly_challenges_fellowship ON weekly_challenges(fellowship_id, status, week_start DESC);
CREATE INDEX IF NOT EXISTS idx_weekly_challenges_active ON weekly_challenges(fellowship_id, status) WHERE status = 'active';
...
CREATE INDEX IF NOT EXISTS idx_challenge_progress_challenge ON challenge_progress(challenge_id, is_completed);
CREATE INDEX IF NOT EXISTS idx_challenge_progress_user ON challenge_progress(user_id, completed_at DESC);
...
-- Badge definitions
CREATE TABLE IF NOT EXISTS blessing_badges (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS user_badges (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  fellowship_id UUID REFERENCES fellowship_groups(id) ON DELETE CASCADE, -- NULL for platform-wide badges
...
CREATE INDEX IF NOT EXISTS idx_user_badges_fellowship ON user_badges(fellowship_id, earned_at DESC);
...
CREATE TABLE IF NOT EXISTS fellowship_highlights (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  fellowship_id UUID REFERENCES fellowship_groups(id) ON DELETE CASCADE NOT NULL,
...
DO $$
BEGIN
    RAISE NOTICE '🎉 Gamification system deployment complete!';
    RAISE NOTICE '✅ All gamification tables created';
    RAISE NOTICE '✅ Challenge templates seeded';
    RAISE NOTICE '✅ Blessing badges seeded';
    RAISE NOTICE '✅ All indexes created';
    RAISE NOTICE '✅ All RLS policies created';
    RAISE NOTICE '✅ All functions created';
    RAISE NOTICE '';
    RAISE NOTICE 'Your app is now ready for gamification! 🔥';
END $$;
